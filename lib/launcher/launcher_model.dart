...
import 'package:laborales/launcher/launcher_model_linux.dart';
import 'package:laborales/launcher/launcher_model_mac.dart';
import 'package:laborales/launcher/launcher_view_model.dart';

/// singleton
final launcherModel = _getLauncherModel();

LauncherModel _getLauncherModel() {
  if (Platform.isMacOS) return LauncherModelMacOS();
  if (Platform.isLinux) return LauncherModelLinux();
  throw AssertionError("OS should be either MacOS or Linux");
}

abstract class LauncherModel {
  Future<List<Project>> loadProjectsFromPrefs();

  Future<void> removeProjectFromPrefs(String projectName);

  Future<void> saveProjectToPrefs(Project project);
